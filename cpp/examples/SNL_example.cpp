...
int main(int argc, char* argv[])
{
 ...
 // 3) Add one UnitSphereD R_k per edge and the LiftedRangePointPoint factor
  ...
  appendRunToResultsJsonFlat(
    "/home/nikolas/StiefelManifold/data/snl/" + dataset_name + "/results.json",
...
//   //   - variables:   L<i> (p-dim vectors), R<k> (UnitSphereD in R^p)
//   //   - factors:     LiftedRangeFactorPointPoint<d>(L_i, L_j, R_k, range, p, noise)
//   //   - init:        random for all L<i> and R<k>
//   // -----------------------------
//   NonlinearFactorGraph inputGraph;
//   Values initial;
//
...
//   }
//
//   // 2) Random-initialize only those landmarks (p-dim vectors)
//   for (size_t lid : landmarkIds) {
//     gtsam::Vector x = gtsam::Vector::Random(p);     // uniform in [-1,1]
//     // (optional Gaussian) for (int j=0;j<p;++j) x(j) = normal(gen);
//     initial.insert(Symbol('L', lid), x);
//   }
//
//   // 3) Add one UnitSphereD R_k per edge and the LiftedRangePointPoint factor
//   size_t k = 0;
...
//
//     // random unit vector in R^p
//     gtsam::Vector rv = gtsam::Vector::Random(p);
//     if (rv.norm() < 1e-12) rv.setConstant(1.0);
//     rv.normalize();
//     UnitSphereD Y(rv);                // adjust ctor if yours differs
//     initial.insert(Rk, Y);
//
//     if (d == 2) {
//       inputGraph.emplace_shared<LiftedRangeFactorPointPoint<2>>(Li, Lj, Rk, m.range, p, noise);
//     } else { // d == 3
//       inputGraph.emplace_shared<LiftedRangeFactorPointPoint<3>>(Li, Lj, Rk, m.range, p, noise);
//     }
//   }
//
//   if (inputGraph.empty()) {
...
//   // -----------------------------
//   auto lmParams = LevenbergMarquardtParams::CeresDefaults();
//   lmParams.maxIterations    = 10000;      // tweak as you wish
//   lmParams.relativeErrorTol = 1e-10;    // tighter tolerances if needed
//   lmParams.verbosityLM      = LevenbergMarquardtParams::SUMMARY;
//
//   std::cout << "[Optimize] Starting LM with " << inputGraph.size()
//             << " factors and " << initial.size() << " variables...\n";
//
//   auto t0 = clock_type::now();
//   LevenbergMarquardtOptimizer lm(inputGraph, initial, lmParams);
