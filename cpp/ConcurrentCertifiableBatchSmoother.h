...
#include "Certifiable_Incremental_PGO.h"

namespace gtsam
{
  inline void printValuesKeys(const gtsam::Values& values,
                              const std::string& header = "Values")
  {
    std::cout << "==== " << header << "  (size = " << values.size() << ") ====\n";
    std::size_t i = 0;
    for (const auto& kv : values) {
      gtsam::Symbol sym(kv.key);         // decode ‘X’, ‘L’, ‘A’, … and numeric index
  ...
  /// Print the (loop-counter, Symbol) and raw 64-bit key for every key that occurs
  /// in any factor inside `graph` (order is arbitrary because KeySet is unordered).
  inline void printGraphKeys(const gtsam::NonlinearFactorGraph& graph,
                             const std::string& header = "Graph keys")
  {
    const gtsam::KeySet keys = graph.keys();        // collects *all* keys in factors
    std::cout << "==== " << header << "  (size = " << keys.size() << ") ====\n";
    std::size_t i = 0;
